...
def signup(request):
    # when user is submitting the signup form after adding their details.
    ...
    # when user is asking for the signup form
    return render(request, "signup.html", {"form": SignUpForm})
...
def login_view(request):
    # check a POST request for submitted a form
    if request.method == 'POST':
        username = request.POST.get('username')
        password = request.POST.get('password')
